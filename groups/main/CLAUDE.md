...
**IMPORTANT: Mount Allowlist Sync**

Additional mounts are validated against an external allowlist at `~/.config/nanoclaw/mount-allowlist.json` on the host. When adding, changing, or removing `additionalMounts`, you **must update both places**:

1. **DB** (`registered_groups.container_config`) — the mount definition
2. **Allowlist** (`~/.config/nanoclaw/mount-allowlist.json`) — the `allowedRoots` entry that permits the host path

If the allowlist doesn't include the host path (or a parent directory), the mount is silently rejected. The allowlist structure:
...
The allowlist is **cached in memory** — changes require a NanoClaw restart to take effect. After updating both DB and allowlist, tell the user to restart NanoClaw.
